    ...
    while (!keysToProcess.empty())
    {
        ...
        if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, currentKey.c_str(), 0, KEY_READ, &hSubKey) == ERROR_SUCCESS)
        {
            ...
            while (true)
            {
                ...
                strncpy_s(item.registryComponentName, DD_SETTINGS_MAX_COMPONENT_NAME_SIZE, parent.c_str(), DD_SETTINGS_MAX_COMPONENT_NAME_SIZE - 1);
                ...
                {
                    strncpy_s(item.settingNameStr, DD_SETTINGS_MAX_MISC_STRING_SIZE, keyName, DD_SETTINGS_MAX_MISC_STRING_SIZE - 1);
