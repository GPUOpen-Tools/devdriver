...
public:
    ...
    DD_RESULT QueryKernelSettingsBlobsAll(DDGpuId        gpuId,
                                          DynamicBuffer* pOutBuf);

    DD_RESULT QueryAllCurrentKernelValues(DDGpuId        gpuId,
                                          DynamicBuffer* pOutBuf);

    DD_RESULT WriteKernelSettingOverride(DDGpuId        gpuId,
                                         uint8_t        type,
                                         uint64_t       numericValue,
                                         const char*    pSettingName,
                                         const char*    pStringValue);

    DD_RESULT TriggerKernelPnpReload(DDGpuId gpuId);
};
