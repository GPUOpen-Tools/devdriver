...
bool EventParser::CopyToTokenBuffer(size_t tokenItemSize)
{
    ...
    if (sizeToCopy > 0)
    {
        Platform::Memcpy_s(
            ...
            (sizeof(m_tokenBuffer) - m_tokenDataSize),
            (m_pBuffer + m_cursor),
            sizeToCopy);
...
DD_EVENT_PARSER_STATE EventParser::Parse()
{
    ...
    while (!yield)
    {
        ...
        case ParserInternalState::ParsingToken:
        {
            ...
            EventTokenType tokenType = static_cast<EventTokenType>(tokenHeader->bits.id);
...
DDEventParserEventInfo EventParser::GetEventInfo()
{
    ...
    info.timestamp = m_currTimestamp + pHeader->bits.delta * DevDriver::kEventTimeUnit;
