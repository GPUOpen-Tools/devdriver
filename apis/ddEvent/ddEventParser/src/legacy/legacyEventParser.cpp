...
Result EventParser::Parse(const void* pEventData, size_t eventDataSize)
{
    ...
    while (reader.HasBytes() && (result == Result::Success))
    {
        ...
        case EventDataState::WaitingForToken:
        {
            ...
            if (result == Result::Success)
            {
                const size_t tokenSize = GetTokenSize(static_cast<EventTokenType>(pTokenHeader->bits.id));
                ...
                if (result == Result::Success)
                {
                    ...
                    {
                        if (pTokenHeader->bits.id == static_cast<uint8>(EventTokenType::Data))
                        {
                        ...
                        else if (pTokenHeader->bits.id == static_cast<uint8>(EventTokenType::TimeDelta))
                        {
        ...
        case EventDataState::WaitingForPayload:
        {
            ...
            if (result == Result::Success)
            {
                if (pTokenHeader->bits.id == static_cast<uint8>(EventTokenType::TimeDelta))
                {
                ...
                else if (pTokenHeader->bits.id == static_cast<uint8>(EventTokenType::Data))
                {
...
void EventParser::WriteIntoTokenBuffer(const void* pData, size_t dataSize)
{
    ...
    Platform::Memcpy_s((m_eventTokenBuffer + m_eventTokenBufferSize),
                       (sizeof(m_eventTokenBuffer) - m_eventTokenBufferSize),
                       pData,
                       dataSize);
...
void EventParser::ProcessToken()
{
    ...
    switch (static_cast<EventTokenType>(pTokenHeader->bits.id))
    {
    case EventTokenType::Provider:
    {
    ...
    case EventTokenType::TimeDelta:
    {
        ...
        Platform::Memcpy_s(&currentTimeDelta, sizeof(currentTimeDelta), pTimeDeltaMemory, numBytes);
