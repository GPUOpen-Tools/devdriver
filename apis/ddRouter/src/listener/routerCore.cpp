...
namespace DevDriver
{
        ...
        // Process system messages
            if (messageHeader.protocolId == Protocol::System)
            {
                ...
                case SystemMessage::ClientInfo:
                {
                    ...
                    if (pSrcClientInfo != nullptr)
                    {
                        const ClientInfoStruct* DD_RESTRICT pPayload =
                            reinterpret_cast<const ClientInfoStruct*>(&message.payload[0]);
                        Platform::Strncpy(pSrcClientInfo->clientInfo.clientName,
                                          pPayload->clientName,
                                          sizeof(pSrcClientInfo->clientInfo.clientName));
                        Platform::Strncpy(pSrcClientInfo->clientInfo.clientDescription,
                                          pPayload->clientDescription,
                                          sizeof(pSrcClientInfo->clientInfo.clientDescription));
    ...
    void RouterCore::UpdateClients()
    {
        ...
        if ((currentTimeInMs - kClientDiscoveryIntervalInMs) >= m_lastClientPingTimeInMs)
        {
            ...
            for (auto it = m_clientMap.begin(); it != m_clientMap.end(); )
            {
                ...
                if (it->second.pingRetryCount > m_clientTimeoutCount)
                {
    ...
    RouterCore::RouterCore() :
        ...
        m_clientTimeoutCount(RouterCore::kClientTimeoutCount),
        m_clientThread(),
    ...
    Result RouterCore::Start(const char* pDescription)
    {
        ...
        if ((m_pClientManager != nullptr) & (m_transportMap.size() > 0))
        {
            ...
            Platform::Strncpy(pPayload->clientDescription, pDescription, sizeof(pPayload->clientDescription));
            ...
            DD_PRINT(LogLevel::Info, "[RouterCore] Started client management thread!");
    ...
    Result RouterCore::SetClientManager(IClientManager *pClientManager)
    {
        ...
        if (m_pClientManager == nullptr)
        {
            ...
            if (result == Result::Success)
            {
                // We should never get an invalid client id back from a successful RegisterHost call
                DD_ASSERT(m_clientId != kBroadcastClientId);

                m_pClientManager = pClientManager;

                DD_PRINT(LogLevel::Info, "[RouterCore] Registered client manager: %s", pClientManager->GetClientManagerName());
            }
        }
        return result;
    }
    ...
    Result RouterCore::RegisterTransport(const std::shared_ptr<IListenerTransport> &pTransport)
    {
        ...
        {
            DD_PRINT(LogLevel::Info, "[RouterCore] Registered transport: %s", pTransport->GetTransportName());
        ...
        {
            DD_PRINT(LogLevel::Error, "[RouterCore] Failed to register transport: %s", pTransport->GetTransportName());
        }
    ...
    Result RouterCore::RemoveTransport(const std::shared_ptr<IListenerTransport> &pTransport)
    {
...

        {
            ...
            if (find != m_transportMap.end())
            {
                ...
                DD_PRINT(LogLevel::Info, "[RouterCore] Removing transport: %s", pTransport->GetTransportName());
                result = Result::Success;
            }
        }
    ...
    void RouterCore::Stop()
    {
        ...
        DD_PRINT(LogLevel::Info, "[RouterCore] Shutting down transport threads...");
        ...
        DD_PRINT(LogLevel::Info, "[RouterCore] Shutting down client management thread...");
        ...
        if (m_clientThread.active)
        {
            ...
            DD_PRINT(LogLevel::Info, "[RouterCore] Client management thread successfully shut down!");
        }
