    ...
    size_t*  pBytesReceived)
{
    ...
    if (result == DD_RESULT_SUCCESS)
    {
        ...
        if ((bytesRemaining > 0) && (pClientContext->payloadCacheSize > 0))
        {
            ...
            Platform::Memcpy_s(pBuffer, bytesRemaining, pDataPtr, bytesToReceive);
        ...
        else if (bytesRemaining > 0)
        {
            ...
            if (result == DD_RESULT_SUCCESS)
            {
                ...
                void* pDataPtr = VoidPtrInc(pBuffer, bytesReceived);
                Platform::Memcpy_s(pDataPtr, bytesRemaining, pClientContext->payloadCache, bytesToReceive);
...
DD_RESULT ddSocketReceiveWithSizePrefixWithTimeout(
    DDSocket  hSocket,
    void*     pBuffer,
    size_t    bufferSize,
    uint64_t* pSizePrefix,
    uint32_t  timeoutMillis)
{
    ...
    if (pSizePrefix != nullptr)
    {
        ...
        result = ddSocketReceiveWithTimeout(hSocket, &sizePrefix, sizeof(sizePrefix), timeoutMillis);
        if (result == DD_RESULT_SUCCESS)
        {
            if (sizePrefix <= SIZE_MAX)
            {
