...
/// Same as ddSocketReceiveWithSizePrefix, but with a 32-bit timeout value which indicates the
/// time to wait before the data-receiving operation times out.
DD_RESULT ddSocketReceiveWithSizePrefixWithTimeout(
    DDSocket    hSocket,      /// [in] Socket handle
    void*       pBuffer,      /// [out] Buffer to write data to
    size_t      bufferSize,   /// Size of the buffer in bytes
    uint64_t*   pSizePrefix,  /// [out] Received size prefix value
    uint32_t    timeoutMillis /// [in] Timeout in milliseconds
);
