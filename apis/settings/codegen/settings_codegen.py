...
def init_scope_string(is_pal: bool, setting: dict, skip_registry: bool) -> str:
...
def check_registry(is_pal: bool, setting: dict, skip_registry: bool) -> str:

    output = ""
    whitelisted = setting.get("Whitelist", False)
    name = setting.get("Name", "")
    if skip_registry and not whitelisted:
        output += "\n#if defined(_WIN32) && _DEBUG && PAL_DISABLE_REGISTRY\n"
        output += "    if (regResult == true)\n"
        output += "    {\n"
        output += "        MessageBoxA(nullptr,"
        output += f" \"A setting override was found in the registry."
        output += " Registry overrides for non-whitelisted settings are now ignored, override will NOT be applied\""
        output += ", \"Setting found in registry!\", MB_OK);\n"
        output += "    }\n"
        output += "#endif // defined(_WIN32) && _DEBUG && PAL_DISABLE_REGISTRY\n"
        ...
        if defaults:
            ...
            if setting_type == "string":
                ...
                assignment_line = (
                    ...
                    f'        Util::Strncpy(m_settings.{setting_name}, "{default_str}", {string_len} - 1);\n'
...
def prepare_settings_meta(
    ...
    magic_buf_start = int.from_bytes(hashlib.sha256(json.dumps(settings_root).encode('UTF-8')).digest()[-4:],
                                     byteorder="big")
    ...
    settings_root["SettingsBlobHash"] = int.from_bytes(hashlib.sha256(settings_blob).digest()[-8:],
                                                       byteorder="big")
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        required=False,
        help="If this string is present, this string will be added into json data.",
    )
    ...
    if args.security_string != "":
        settings_root["SecurityString"] = args.security_string
    ...
    if settings_root["ComponentName"] in ["oglp"]:
        settings_root["IsOglp"] = True
    ...
    jinja_env.filters["check_registry"] = check_registry
    ...
    with open(g_header_path, "w") as generated_file:
        generated_file.write(header_template.render(settings_root, current_year=datetime.now().year))

    with open(g_source_path, "w") as generated_file:
        generated_file.write(source_template.render(settings_root, current_year=datetime.now().year))
