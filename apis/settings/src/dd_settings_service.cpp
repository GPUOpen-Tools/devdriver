...
void SettingsRpcService::UnRegisterSettingsComponent(SettingsBase* pSettingsComponent)
{
    LockGuard lock(m_settingsComponentsMutex);

    Result result = m_settingsComponents.Erase(pSettingsComponent->GetComponentName());
    ...
    if (pOverrides != nullptr)
    {
        for (const DDSettingsValueRef& override : *pOverrides)
        {
            if (override.hash == nameHash)
            {
                ...
                {
                    DevDriver::Platform::Memcpy_s(pSetting, settingSize, override.pValue, override.size);
                    applied = true;
                }
                break;
            }
...
DD_RESULT SettingsRpcService::SendAllUserOverrides(const void* pParamBuf, size_t paramBufSize)
{
    ...
    DevDriver::Platform::Memcpy_s(m_pAllUserOverridesData, paramBufSize, pParamBuf, paramBufSize);
...
DD_RESULT SettingsRpcService::QueryAllCurrentValues(const DDByteWriter& writer)
{
    ...
    for (const auto& entry : m_settingsComponents)
    {
        ...
        if (result == DD_RESULT_SUCCESS)
        {
            ...
            size_t compNameSize = Platform::Strlen_s(entry.value->GetComponentName(), DD_SETTINGS_MAX_COMPONENT_NAME_SIZE - 1) + 1; // + 1 for null-terminator
            ...
            compNameSize = (compNameSize > DD_SETTINGS_MAX_COMPONENT_NAME_SIZE) ? DD_SETTINGS_MAX_COMPONENT_NAME_SIZE
                                                                                : compNameSize;
            DevDriver::Platform::Memcpy_s(header.name, DD_SETTINGS_MAX_COMPONENT_NAME_SIZE, entry.value->GetComponentName(), compNameSize);
