...
inline void SetValueHelper(DDSettingsValueRef* pDestValueRef, const DDSettingsValueRef& pSrcValueRef)
{
    if (pDestValueRef->isOptional)
    {
        ...
        case DD_SETTINGS_TYPE_STRING: DevDriver::Platform::Memcpy_s(pDestValueRef->pValue, pDestValueRef->size, pSrcValueRef.pValue, pSrcValueRef.size); break;
        default:
            DD_ASSERT(false);
        }
    }
...
DD_RESULT SettingsBase::GetValue(DDSettingsValueRef* pValueRef)
{
    ...
    if (pSrcValueRef != nullptr)
    {
        if (pValueRef->size >= pSrcValueRef->size)
        {
            ...
            {
                DevDriver::Platform::Memcpy_s(pValueRef->pValue, pValueRef->size, pSrcValueRef->pValue, pSrcValueRef->size);
