...
#define DD_SETTINGS_API_VERSION_MINOR 3
...
typedef enum
{
    ...
    DD_SETTINGS_DRIVER_TYPE_KMD,
    DD_SETTINGS_DRIVER_TYPE_UNKNOWN,
...
typedef struct DDSettingsApi
{
    ...
    /// Retrieve settings blobs of all components from the kernel.
    ///
    ///
    /// @param[in]  pInstance Must be \ref DDSettingsApi.pInstance.
    /// @param[out] ppSettingsBlobs Will be set to a pointer to the buffer to receive settings blobs.
    /// @param[out] pSettingsBlobsSize Will be set to the size of the buffer of settings blobs.
    /// @param[in]  alloc Used to allocate the buffer to receive settings blobs.
    ...
    /// Query the values of settings in all components from the kernel.
    ///
    /// @param[in]  pInstance Must be \ref DDSettingsApi.pInstance.
    /// @param[in]  gpuId The GPU ID to query settings blobs for.
    /// @param[out] ppBuffer Will be set to a pointer to a byte-array containing settings values.
    /// @param[out] pSize The size of the byte-array pointed to by *\param ppBuffer.
    /// @param[in]  alloc Will be used to allocate the byte-array mentioned above.
    ...
    /// Write a single KMD setting override to the registry on the machine running RDS.
    /// Windows-only; returns DD_RESULT_COMMON_UNSUPPORTED on other platforms.
    ///
    /// @param[in] pInstance     Must be \ref DDSettingsApi.pInstance.
    /// @param[in] gpuId         GPU to target (bus<<16 | device<<8 | function).
    /// @param[in] type          DD_SETTINGS_TYPE of the value.
    /// @param[in] numericValue  Raw 64-bit representation of the value (unused for string type).
    /// @param[in] pSettingName  Null-terminated setting name string.
    /// @param[in] pStringValue  Null-terminated string value (may be NULL for non-string types).
    ...
    /// Perform a PnP disable/re-enable cycle on a GPU to reload the KMD and pick up registry overrides.
    /// Windows-only; returns DD_RESULT_COMMON_UNSUPPORTED on other platforms.
    ///
    /// @param[in] pInstance Must be \ref DDSettingsApi.pInstance.
    /// @param[in] gpuId     GPU to reload (bus<<16 | device<<8 | function).
