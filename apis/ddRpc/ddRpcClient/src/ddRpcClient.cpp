...
DD_RESULT ddRpcClientGetServiceInfo(DDRpcClient hClient, const DDRpcServiceId serviceId, DDApiVersion* pVersion)
{
        ...
        ValidateOptionalBuffer(info.pParamBuffer, info.paramBufferSize))
    {
        ...
        if (result == DD_RESULT_SUCCESS)
        {
            ...
            // Static assertion to ensure this code is updated if DDApiVersion structure changes
            static_assert(sizeof(DDApiVersion) == sizeof(uint32_t) * 3,
                          "DDApiVersion structure has changed. Update field assignments in ddRpcClientGetServiceInfo.");
            const DDApiVersion* pSrcVersion = static_cast<const DDApiVersion*>(writer.Buffer());
            pVersion->major = pSrcVersion->major;
            pVersion->minor = pSrcVersion->minor;
            pVersion->patch = pSrcVersion->patch;
        }
    }
