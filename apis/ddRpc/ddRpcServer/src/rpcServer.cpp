    ...
    const DDRpcServerRegisterServiceInfo& info)
{
    ...
    // Note: Byte extraction assumes little-endian (x86/x64/ARM) platforms.
    DD_PRINT(LogLevel::Info, "Registering service: id=0x%x ('%c%c%c%c'), name=\"%s\"",
             info.id,
             static_cast<char>((info.id >> 0) & 0xFF),
             static_cast<char>((info.id >> 8) & 0xFF),
             static_cast<char>((info.id >> 16) & 0xFF),
             static_cast<char>((info.id >> 24) & 0xFF),
             info.pName);
