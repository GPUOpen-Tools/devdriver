	...
	*/
    ...
    public:
        ...
        ChunkId(const char* id)
        {
            ...
            rdf::platform::memcpy_s(id_, sizeof(id_), id, size);
        }

        ChunkId(const ChunkId& rhs)
        {
            rdf::platform::memcpy_s(id_, sizeof(id_), rhs.id_, sizeof(rhs.id_));
        }

        ChunkId& operator=(const ChunkId& rhs)
        {
            rdf::platform::memcpy_s(id_, sizeof(id_), rhs.id_, sizeof(IdType));
            return *this;
        }
    ...
    private:
        ...
        private:
            ...
            {
                if (name) {
                    rdf::platform::memcpy_s(name, sizeof(it_->first), &it_->first, sizeof(it_->first));
                }
    ...
    public:
                        ...
                        const std::uint32_t version)
        {
            ...
            rdf::platform::memcpy_s(entry.chunkIdentifier, sizeof(entry.chunkIdentifier), chunkIdentifier,
    ...
    private:
        void Construct(bool append)
        {
            ...
            if (append) {
                ...
                rdf::platform::memcpy_s(header_.identifier, sizeof(header_.identifier), ChunkFile::Identifier, sizeof(ChunkFile::Identifier));
    ...
    */
    ...
    private:
            ...
            const std::int64_t count, void* buffer) override
        {
            ...
            rdf::platform::memcpy_s(buffer, count,
    ...
    */
    ...
    private:
            ...
            const std::int64_t count, void* buffer) override
        {
            ...
            rdf::platform::memcpy_s(buffer, count, data_.data() + offset, bytesToRead);
            return bytesToRead;
        }
            ...
            const std::int64_t count, const void* buffer) override
        {
            ...
            rdf::platform::memcpy_s(data_.data() + offset, data_.size() - offset, buffer, count);
            return count;
        }
...
#if RDF_PLATFORM_UNIX
        // On Unix, when creating a file, use open() with explicit restrictive permissions (0600)
        // to avoid creating world-writable files. This is more secure than relying on umask.
        if (fileMode == rdfFileModeCreate && accessMode == rdfStreamAccessReadWrite) {
            // Create file with read/write permissions only for the owner (0600)
            int file_descriptor = ::open(filename, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
            if (file_descriptor == -1) {
                throw std::runtime_error("Could not create file");
            }
            fd = ::fdopen(file_descriptor, mode);
            if (fd == nullptr) {
...
#if RDF_PLATFORM_UNIX
        // On Unix, create file with read/write permissions only for the owner (0600)
        // to avoid creating world-writable files. This is more secure than relying on umask.
        int file_descriptor = ::open(filename, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
        if (file_descriptor == -1) {
            throw std::runtime_error("Could not create file");
        }
        fd = ::fdopen(file_descriptor, "w+b");
        if (fd == nullptr) {
            ::close(file_descriptor);
            throw std::runtime_error("Could not create file");
        }
#else
        // Use _sopen_s with explicit permissions to avoid creating world-writable files.
        int file_descriptor = -1;
        errno_t err = ::_sopen_s(&file_descriptor,
                                 filename,
                                 _O_RDWR | _O_CREAT | _O_TRUNC | _O_BINARY,
                                 _SH_DENYNO,
                                 _S_IREAD | _S_IWRITE);
        if ((err != 0) || (file_descriptor == -1)) {
            throw std::runtime_error("Could not create file");
        }
        fd = ::_fdopen(file_descriptor, "w+b");
        if (fd == nullptr) {
...
int RDF_EXPORT rdfStreamFromFile(const rdfStreamFromFileCreateInfo* info, rdfStream** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfStream>(new rdfStream);
...
int RDF_EXPORT rdfStreamOpenFile(const char* filename, rdfStream** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfStream>(new rdfStream);
        tmp->stream = rdf::internal::OpenFile(filename, rdfStreamAccessRead, rdfFileModeOpen);
        *handle = tmp.release();
    }
...
int RDF_EXPORT rdfStreamCreateFile(const char* filename, rdfStream** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfStream>(new rdfStream);
        tmp->stream = rdf::internal::CreateFile(filename);
        *handle = tmp.release();
    }
                                           ...
                                           rdfStream** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfStream>(new rdfStream);
        tmp->stream = rdf::internal::CreateReadOnlyMemoryStream(size, data);
        *handle = tmp.release();
    }
...
int RDF_EXPORT rdfStreamCreateMemoryStream(rdfStream** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfStream>(new rdfStream);
        tmp->stream = rdf::internal::CreateMemoryStream();
        *handle = tmp.release();
    }
...
int RDF_EXPORT rdfStreamFromUserStream(const rdfUserStream* userStream, rdfStream** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfStream>(new rdfStream);
        tmp->stream = rdf::internal::rdf_make_unique<rdf::internal::UserStream>(*userStream);
        *handle = tmp.release();
    }
...
int RDF_EXPORT rdfChunkFileOpenFile(const char* filename, rdfChunkFile** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfChunkFile>(new rdfChunkFile);
        tmp->chunkFile.reset(new rdf::internal::ChunkFile(
...
int RDF_EXPORT rdfChunkFileOpenStream(rdfStream* stream, rdfChunkFile** handle)
{
    ...
    *handle = nullptr;
    {
        auto tmp = std::unique_ptr<rdfChunkFile>(new rdfChunkFile);
        tmp->chunkFile.reset(new rdf::internal::ChunkFile(stream->stream.get()));
        *handle = tmp.release();
    }
                                               ...
                                               rdfChunkFileIterator** iterator)
{
    ...
    *iterator = nullptr;
    {
        auto tmp = std::unique_ptr<rdfChunkFileIterator>(new rdfChunkFileIterator);
        tmp->iterator = handle->chunkFile->GetIterator();
        *iterator = tmp.release();
    }
...
int RDF_EXPORT rdfChunkFileWriterCreate(rdfStream* stream, rdfChunkFileWriter** writer)
{
    ...
    *writer = nullptr;
    {
        auto tmp = std::unique_ptr<rdfChunkFileWriter>(new rdfChunkFileWriter);
        tmp->writer.reset(new rdf::internal::ChunkFileWriter(stream->stream.get(), false));
        *writer = tmp.release();
    }
...
int RDF_EXPORT rdfChunkFileWriterCreate2(const rdfChunkFileWriterCreateInfo* info, rdfChunkFileWriter** writer)
{
    ...
    *writer = nullptr;
    {
        auto tmp = std::unique_ptr<rdfChunkFileWriter>(new rdfChunkFileWriter);
        tmp->writer.reset(new rdf::internal::ChunkFileWriter(info->stream->stream.get(), info->appendToFile));
        *writer = tmp.release();
    }
