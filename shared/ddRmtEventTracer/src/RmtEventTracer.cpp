...
constexpr char kApiInfoChunkId[RDF_IDENTIFIER_SIZE]  = "ApiInfo";
    ...
    bool                   useKmd)
{
    ...
    if ((result == DD_RESULT_SUCCESS) && (isTargetLinux == false) && (useKmd == true))
    {
    ...
    if ((result == DD_RESULT_SUCCESS) && (isTargetLinux == false) && (useKmd == true))
    {
        ...
        {
            LogInfo("Successfully connected to AmdLog.");
        }
    }
...
DD_RESULT RmtEventTracer::WriteSavedChunks(rdfChunkFileWriter* pRdfChunkWriter)
{
    ...
    // Write heap info chunks here
    {
        ...
        Platform::Memcpy_s(info.identifier, sizeof(info.identifier), kHeapChunkId, RDF_IDENTIFIER_SIZE);
        info.pHeader     = nullptr;
        info.headerSize  = 0;
        info.version     = 1;
        info.compression = rdfCompression::rdfCompressionZstd;
    ...
    {
        rdfChunkCreateInfo info = {};
        Platform::Memcpy_s(info.identifier, sizeof(info.identifier), kAdapterChunkId, RDF_IDENTIFIER_SIZE);
        info.pHeader     = nullptr;
        info.headerSize  = 0;
        info.version     = 1;
        info.compression = rdfCompression::rdfCompressionZstd;
    ...
    if (result == DD_RESULT_SUCCESS)
    {
        for (size_t idx = 0; idx < m_snapshots.Size(); ++idx)
        {
            ...
            rdfChunkCreateInfo info      = {};
            Platform::Memcpy_s(info.identifier, sizeof(info.identifier), kSnapshotChunkId, RDF_IDENTIFIER_SIZE);
            info.pHeader     = nullptr;
            info.headerSize  = 0;
...
DD_RESULT RmtEventTracer::TransferTraceData(
    ...
    rdfChunkFileWriter*      pRdfChunkWriter,
    bool                     useCompression,
    const TraceChunkApiInfo* pApiInfoChunk)
{
    DD_RESULT result = DD_RESULT_SUCCESS;
    ...
    {
        if (pApiInfoChunk != nullptr)
        {
            rdfChunkCreateInfo chunkInfo = {};
            Platform::Memcpy_s(chunkInfo.identifier, sizeof(chunkInfo.identifier), kApiInfoChunkId, RDF_IDENTIFIER_SIZE);
            chunkInfo.headerSize  = 0;
            chunkInfo.pHeader     = nullptr;
            chunkInfo.compression = rdfCompressionNone;
            chunkInfo.version     = 2;

            result = RdfResultToDDResult(rdfChunkFileWriterWriteChunk(
                pRdfChunkWriter,
                &chunkInfo,
                sizeof(*pApiInfoChunk),
                pApiInfoChunk,
                &m_currentChunkIndex));
        }
        ...
        if (result == DD_RESULT_SUCCESS)
        {
            ...
            for (uint32 streamIndex = 0; streamIndex < static_cast<uint32>(m_dataStreams.Size()); ++streamIndex)
            {
                ...
                if (ferror(pStream->pFileHandle) == 0)
                {
                    ...
                    rdfChunkCreateInfo chunkInfo = {};
                    Platform::Memcpy_s(chunkInfo.identifier, sizeof(chunkInfo.identifier), kStreamChunkId, RDF_IDENTIFIER_SIZE);
    ...
    uint64_t    snapshotTimestamp)
{
    ...
    if (m_traceState == TraceState::Running)
    {
        ...
        const uint32 snapshotNameLength = static_cast<uint32>(Platform::Strlen_s(pSnapshotName, kMaxSnapshotNameLen));

        // The identifier name needs to fit
        DD_ASSERT(snapshotNameLength < kMaxSnapshotNameLen);

        TraceSnapShot snapshot = {};
        Platform::Memcpy_s(snapshot.name, sizeof(snapshot.name), pSnapshotName, snapshotNameLength + 1);
...
void RmtEventTracer::DiscardDataStreams()
{
    ...
    Platform::AtomicSet(&m_totalDataSize, 0);
}
