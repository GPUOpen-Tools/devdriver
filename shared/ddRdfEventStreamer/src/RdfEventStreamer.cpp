...
DD_RESULT RdfEventStreamer::EndStreaming(bool isClientAlive)
{
    ...
    {
        Platform::AtomicSet(&m_totalDataSize, static_cast<int64>(ftell(m_pStreamFile)));
    }
    else
    {
        Platform::AtomicSet(&m_totalDataSize, 0);
    ...
    bool                 useCompression)
{
    ...
    {
        const int64 totalDataSize = Platform::AtomicGet(&m_totalDataSize);
        if (totalDataSize > 0)
        {
            ...
            Platform::Memcpy_s(chunkInfo.identifier, RDF_IDENTIFIER_SIZE, kDevDriverEventChunkId, RDF_IDENTIFIER_SIZE);
