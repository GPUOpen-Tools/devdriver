        ...
        void GrowBlocks(size_t numBlocks)
        {
            ...
            if (pNewIndexCache != nullptr)
            {
                ...
                if (m_numBlocks > 0)
                {
                    ...
                    // lambda to calculate the number of bytes remaining in the newly allocated buffer
                    // based on the specified offset
                    const auto bytesRemainingFrom = [&](const QueueBlockPtr* pCursor) -> size_t
                    {
                        const QueueBlockPtr* pEnd = pNewIndexCache + newCapacity;
                        DD_ASSERT((pCursor >= pNewIndexCache) && (pCursor <= pEnd));
                        return static_cast<size_t>(pEnd - pCursor) * sizeof(QueueBlockPtr);
                    };

                    // Copy pointers from head to the end of the existing allocation + increment pointer
                    Platform::Memcpy_s(basePtr, bytesRemainingFrom(basePtr), &m_pBlockIndexCache[blockIndex], sizeof(QueueBlockPtr) * lengthToEnd);
                    basePtr += lengthToEnd;
                    ...
                    if (blockIndex <= count)
                    {
                        Platform::Memcpy_s(basePtr, bytesRemainingFrom(basePtr), &m_pBlockIndexCache[0], sizeof(QueueBlockPtr) * blockIndex);
                        basePtr += blockIndex;
                    }
                    else
                    {
                        // Otherwise we need to split it into two copy operations - the number that can fit at the end
                        // and the number that will have to go at the beginning
                        Platform::Memcpy_s(basePtr, bytesRemainingFrom(basePtr), &m_pBlockIndexCache[0], sizeof(QueueBlockPtr) * count);
                        basePtr += count;
                        Platform::Memcpy_s(&pNewIndexCache[0], bytesRemainingFrom(&pNewIndexCache[0]), &m_pBlockIndexCache[count], sizeof(QueueBlockPtr) * (blockIndex - count));
                    }
                    // Free the original
                    DD_FREE(m_pBlockIndexCache, m_allocCb);
                }
