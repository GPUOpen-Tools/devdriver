...
enum RMT_RESOURCE_TYPE
{
    ...
    RMT_RESOURCE_TYPE_WORKGRAPH              = 17
};
...
struct RMT_MSG_USERDATA_EMBEDDED_STRING : RMT_TOKEN_DATA
{
    ...
    {
        // Store the string
        uint32 payloadSize = static_cast<uint32>(Platform::Strlen_s(pString, RMT_MAX_USERDATA_STRING_SIZE));
        ...
        Platform::Memcpy_s(&bytes[RMT_MSG_USERDATA_TOKEN_BYTES_SIZE],
                           sizeof(bytes) - RMT_MSG_USERDATA_TOKEN_BYTES_SIZE,
                           pString,
                           payloadSize);
    }
};
...
struct RMT_MSG_USERDATA_DEBUG_NAME : RMT_TOKEN_DATA
{
    ...
    RMT_MSG_USERDATA_DEBUG_NAME(uint8 delta, const char* pDebugName, uint32 resourceId)
    {
        ...
        {
            stringSize = static_cast<uint32>(Platform::Strlen_s(pDebugName, RMT_MAX_USERDATA_STRING_SIZE));
        }
        ...
        if (pDebugName)
        {
            ...
            Platform::Memcpy_s(&bytes[RMT_MSG_USERDATA_TOKEN_BYTES_SIZE],
                               BUFFER_SIZE - RMT_MSG_USERDATA_TOKEN_BYTES_SIZE,
                               pDebugName,
                               stringSize);
        }
        ...
        Platform::Memcpy_s(&bytes[RMT_MSG_USERDATA_TOKEN_BYTES_SIZE + stringSize + 1],
                           BUFFER_SIZE - (RMT_MSG_USERDATA_TOKEN_BYTES_SIZE + stringSize + 1),
                           &resourceId,
                           sizeof(resourceId));
    }
};
...
struct RMT_MSG_USERDATA_DEBUG_NAME_V2 : RMT_TOKEN_DATA
{
    ...
    RMT_MSG_USERDATA_DEBUG_NAME_V2(uint8 delta, const char* pDebugName, uint32 resourceId, uint64 lagTime)
    {
        ...
        // Calculate the string storage size
        uint32 stringSize = static_cast<uint32>(Platform::Strlen_s(pDebugName, RMT_MAX_USERDATA_STRING_SIZE));
        ...
        Platform::Memcpy_s(&bytes[RMT_MSG_USERDATA_TOKEN_BYTES_SIZE + stringSize + RMT_ENCODED_RESOURCE_ID_SIZE],
                           BUFFER_SIZE - (RMT_MSG_USERDATA_TOKEN_BYTES_SIZE + stringSize + RMT_ENCODED_RESOURCE_ID_SIZE),
                           &lagTime,
                           sizeof(lagTime));
    }
};
...
public:
    ...
    RMT_MSG_USERDATA_MARK_IMPLICIT_RESOURCE(uint8 delta, uint32 resourceId)
    {
        ...
        Platform::Memcpy_s(pPayload, PAYLOAD_SIZE, &resourceId, sizeof(resourceId));
    }
};
...
public:
    ...
    RMT_MSG_USERDATA_MARK_IMPLICIT_RESOURCE_V2(uint8 delta, uint32 resourceId, uint64 lagTime, uint8 heapType)
    {
        ...
        // Insert the time delay into the payload after the resource id.
        Platform::Memcpy_s(&bytes[RMT_MSG_USERDATA_TOKEN_BYTES_SIZE + RMT_ENCODED_RESOURCE_SIZE],
                           PAYLOAD_SIZE - RMT_ENCODED_RESOURCE_SIZE,
                           &lagTime,
                           sizeof(lagTime));
        ...
        Platform::Memcpy_s(&bytes[RMT_MSG_USERDATA_TOKEN_BYTES_SIZE + RMT_ENCODED_RESOURCE_SIZE +
                                  RMT_ENCODED_NAME_EVENT_TIME_DELAY_SIZE],
                           PAYLOAD_SIZE - (RMT_ENCODED_RESOURCE_SIZE + RMT_ENCODED_NAME_EVENT_TIME_DELAY_SIZE),
                           &heapType,
                           sizeof(heapType));
    }
};
...
public:
    ...
    RMT_MSG_USERDATA_RSRC_CORRELATION(uint8 delta, uint32 handle, uint32 dxHandle)
    {
        ...
        Platform::Memcpy_s(pPayload, PAYLOAD_SIZE, &handle, sizeof(handle));
        pPayload += sizeof(handle);
        Platform::Memcpy_s(pPayload, PAYLOAD_SIZE - sizeof(handle), &dxHandle, sizeof(dxHandle));
    }
};
