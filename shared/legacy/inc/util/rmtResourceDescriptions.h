...
enum RMT_ENGINE_TYPE
{
    ...
    RMT_ENGINE_TYPE_BRIGHTON_DECODE               = 11,
    RMT_ENGINE_TYPE_BRIGHTON_ENCODE               = 12,
    RMT_ENGINE_TYPE_BRIGHTON_JPEG_DECODE          = 13,
    RMT_ENGINE_TYPE_BRIGHTON_ENCRYPTION           = 14,
    RMT_ENGINE_TYPE_BRIGHTON_MOTION_ESTIMATION    = 15,
    RMT_ENGINE_TYPE_BRIGHTON_MOTION_ESTIMATION_ML = 16,
};
...
// Bitfield defining WorkGraph creation flags
struct RMT_WORKGRAPH_CREATE_FLAGS
{
    union
    {
        struct
...
// Structure defining a 128-bit WorkGraph hash
struct RMT_WORKGRAPH_HASH
{
    uint64 hashUpper;
    uint64 hashLower;
};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// RMT_RESOURCE_TYPE_WORKGRAPH
static const size_t RMT_WORKGRAPH_BYTES_SIZE = 136 / 8; // 136-bits
struct RMT_RESOURCE_TYPE_WORKGRAPH_TOKEN : RMT_TOKEN_DATA
{
    uint8 bytes[RMT_WORKGRAPH_BYTES_SIZE];

    // Initializes the token fields
    RMT_RESOURCE_TYPE_WORKGRAPH_TOKEN(RMT_WORKGRAPH_CREATE_FLAGS flags, RMT_WORKGRAPH_HASH hash)
    {
        sizeInBytes = sizeof(bytes);
        pByteData = &bytes[0];

        // FLAGS [7:0] Describes the creation flags for the WorkGraph.
        SetBits(flags.byteVal, 7, 0);

        // WORKGRAPH_HASH [135:8] The 128bit pipeline hash of the code object.
        SetBits(hash.hashUpper, 71, 8);
        SetBits(hash.hashLower, 135, 72);
    }
};
