...
namespace DevDriver
{
...
#if defined(DD_ENABLE_UWP_TRANSPORT)
        else if (m_createInfo.connectionInfo.type == TransportType::MessageBus)
        {
            using MsgChannelLocal = MessageChannel<LocalMsgTransport>;
            m_pMsgChannel = DD_NEW(MsgChannelLocal, m_allocCb)(m_allocCb,
                                                               m_createInfo,
                                                               m_allocCb,
                                                               m_createInfo.componentType,
                                                               m_createInfo.initialFlags);
        }
#endif
        else if (m_createInfo.connectionInfo.type == TransportType::LocalNg)
        {
            using MsgChannelLocalNg = MessageChannel<LocalNgMsgTransport>;
            m_pMsgChannel = DD_NEW(MsgChannelLocalNg, m_allocCb)(m_allocCb,
                                                                 m_createInfo,
                                                                 m_allocCb,
                                                                 m_createInfo.componentType,
                                                                 m_createInfo.initialFlags);
    ...
    void DevDriverServer::Finalize()
    {
        ...
        if (m_createInfo.servers.bits.driverControl)
#else
...
#if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
        if (m_createInfo.servers.bits.rgp)
#else
...
#if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
        if (m_createInfo.servers.bits.event)
#else
...
#if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
        const bool enableDriverControl = m_createInfo.servers.bits.driverControl;
        const bool enableRgp           = m_createInfo.servers.bits.rgp;
        const bool enableEvent         = m_createInfo.servers.bits.event;
#else
        const bool enableDriverControl = m_createInfo.servers.driverControl;
        const bool enableRgp           = m_createInfo.servers.rgp;
        const bool enableEvent         = m_createInfo.servers.event;
#endif

        if ((result == Result::Success) && enableDriverControl)
        {
            result = RegisterProtocol<Protocol::DriverControl>();
        }

        if ((result == Result::Success) && enableRgp)
        {
            result = RegisterProtocol<Protocol::RGP>();
        }

        if ((result == Result::Success) && enableEvent)
        {
                                                ...
                                                uint32          timeout)
    {
        ...
        switch (hostInfo.type)
        {
            case TransportType::Local:
                ...
                break;
#if defined(DD_ENABLE_UWP_TRANSPORT)
            case TransportType::MessageBus:
                // UWP support is provided through the message bus transport
                // Ignores the timeout since it isn't valid
                result = LocalMsgTransport::TestConnection(Platform::GenericAllocCb);
                break;
#endif

#if defined(DD_PLATFORM_WINDOWS_UM)
            case TransportType::LocalNg:
                result = LocalNgMsgTransport::TestConnection(Platform::GenericAllocCb);
                break;
#endif
            default:
