...
namespace DevDriver
{
        ...
        if (m_pMsgChannel != nullptr)
        {
            if (strcmp(pContext->GetRequestArguments(), "info") == 0)
            {
                ...
                if (result == Result::Success)
                {
                    ...
                    const bool supportsDriverControl = clientInfo.metadata.protocols.bits.driverControl;
    #else
                    const bool supportsDriverControl = clientInfo.metadata.protocols.driverControl;
    #endif
                    pResponse->Write("\nClient Driver Control Protocol Support: %u", supportsDriverControl);

                    if (supportsDriverControl)
                    {
    ...
    #if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
                    const bool supportsRgp = clientInfo.metadata.protocols.bits.rgp;
    #else
                    const bool supportsRgp = clientInfo.metadata.protocols.rgp;
    #endif
                    pResponse->Write("\nClient RGP Protocol Support: %u", supportsRgp);
    ...
    #if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
                    const bool supportsEvent = clientInfo.metadata.protocols.bits.event;
    #else
                    const bool supportsEvent = clientInfo.metadata.protocols.event;
    #endif
                    pResponse->Write("\nClient ETW Protocol Support: %u", supportsEvent);
