        ...
        if (messageBuffer.header.payloadSize <= kMaxPayloadSizeInBytes)
        {
            ...
            if (messageBuffer.header.sequence >= nextSequence)
            {
                ...
                if (distance < m_receiveWindow.GetWindowSize())
                {
                    ...
                    Memcpy_s(&m_receiveWindow.messages[index],
        ...
        if (m_sessionState < SessionState::FinWait2)
        {
            if (payloadSizeInBytes <= kMaxPayloadSizeInBytes)
            {
                ...
                if (result == Result::Success)
                {
                    ...
                    {
                        Memcpy_s(&messageBuffer.payload[0], sizeof(messageBuffer.payload), pPayload, payloadSizeInBytes);
                    }
            ...
            else if (result == Result::Success)
            {
                ...
                if (payloadBufferSizeInBytes >= message.header.payloadSize)
                {
                    if (static_cast<SessionMessage>(message.header.messageId) == SessionMessage::Data)
                    {
                        ...
                        Memcpy_s(pPayloadBuffer, payloadBufferSizeInBytes, &message.payload[0], payloadSize);
