...
URIRequestPayload::URIRequestPayload(
    const char*        pRequest,
    BlockId            block,
    TransferDataFormat dataFormat,
    uint32             size)
...
URIResponsePayload::URIResponsePayload(
    Result             status,
    BlockId            block,
    TransferDataFormat format,
    uint32             size)
...
URIPostRequestPayload::URIPostRequestPayload(const char* pRequest, uint32 size)
...
URIPostResponsePayload::URIPostResponsePayload(Result status, BlockId block)
        ...
        (pRequestString != nullptr))
    {
        ...
        else if (pPostData != nullptr)
        {
            ...
            if (postDataSize <= kMaxInlineDataSize)
            {
                ...
                Platform::Memcpy_s(pPostDataLocation, kMaxInlineDataSize, pPostData, postDataSize);
