...
EventChunkBufferView::EventChunkBufferView(EventChunk** ppChunkList, size_t numChunks)
...
EventChunkBufferView::EventChunkBufferView(EventChunk** ppChunk)
...
Result EventChunkBufferView::WriteEventProviderToken(
    EventProviderId providerId,
    uint64          frequency,
    uint64          timestamp)
{
    EventTokenHeader header = {};
    header.bits.id          = static_cast<uint8>(EventTokenType::Provider);
    header.bits.delta       = 0;
...
Result EventChunkBufferView::WriteEventDataToken(
    uint8  delta,
    uint32 eventId,
    uint32 index,
...
{
    EventTokenHeader header = {};
    header.bits.id          = static_cast<uint8>(EventTokenType::Data);
    header.bits.delta       = delta;
...
Result EventChunkBufferView::WriteEventTimestampToken(
    uint64 frequency,
    uint64 timestamp)
{
    EventTokenHeader header = {};
    header.bits.id          = static_cast<uint8>(EventTokenType::Timestamp);
    header.bits.delta       = 0;
...
Result EventChunkBufferView::WriteEventTimeDeltaToken(
    uint8  numBytes,
    uint64 timeDelta)
{
    ...
    EventTokenHeader header = {};
    header.bits.id          = static_cast<uint8>(EventTokenType::TimeDelta);
    header.bits.delta       = 0;
