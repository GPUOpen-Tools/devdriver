    ...
    switch (pSessionData->state)
    {
        ...
        case SessionState::StepDriver:
        {
            ...
            if ((IsHalted() && (m_initStepRequested == false)) ||
                (IsDriverInitialized() && (Platform::AtomicGet(&m_stepCounter) == 0)))
            {
    ...
    const Version          sessionVersion)
{
    ...
    if (((m_driverStatus == DriverStatus::Paused) ||
        ...
        Platform::AtomicGet(&m_stepCounter) == 0)
    {
        int32 count = Platform::Max((int32)payload.count, 1);
        Platform::AtomicAdd(&m_stepCounter, count);
        DD_PRINT(LogLevel::Verbose, "[DriverControlServer] Stepping driver %i frames", Platform::AtomicGet(&m_stepCounter));
...
void DriverControlServer::DriverTick()
{
    ...
    LockData();
    if (IsDriverInitialized() && (Platform::AtomicGet(&m_stepCounter) > 0))
    {
        const int32 remaining = Platform::AtomicDecrement(&m_stepCounter);
        DD_PRINT(LogLevel::Verbose, "[DriverControlServer] %i frames remaining", remaining);
...
void DriverControlServer::WaitForResume()
{
    ...
    while (waitResult == Result::NotReady)
    {
        ...
        if (Platform::AtomicGet(&m_numSessions) == 0)
        {
