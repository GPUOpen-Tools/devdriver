                ...
                while (readResult == Result::Success && !traceCompleted)
                {
                    switch (payload.command)
                    {
                        case ETWMessage::TraceDataChunk:
                        {
                            ...
                            {
                                const size_t bytesToCopy = payload.traceDataChunk.numEvents * sizeof(GpuEvent);
                                Platform::Memcpy_s(&buffer[numEventsCopied],
                                                   (numEvents - numEventsCopied) * sizeof(GpuEvent),
                                                   &payload.traceDataChunk.events[0],
                                                   bytesToCopy);
