...
namespace DevDriver
{
    ...
    Result MessageChannel<MsgTransport>::RegisterProtocolServer(IProtocolServer* pServer)
    {
        ...
        if (pServer != nullptr)
        {
            ...
            case Protocol::DriverControl:
            {
                ...
                enabledProtocols.bits.driverControl = true;
#else
            ...
            {
#if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
                enabledProtocols.bits.rgp      = true;
#else
            ...
            {
#if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >=GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
                enabledProtocols.bits.etw      = true;
#else
            ...
            {
#if GPUOPEN_CLIENT_INTERFACE_MAJOR_VERSION >= GPUOPEN_INTERFACE_EXHIBIT_G_REFACTOR
                enabledProtocols.bits.event      = true;
#else
                    ...
                    if (m_discoveredClientsQueue.active)
                    {
                        if (m_discoveredClientsQueue.filter.Matches(metadata))
                        {
                            ...
                            else if (messageBuffer.header.payloadSize == sizeof(clientInfo.clientInfo.data))
                            {
                                ...
                                Platform::Memcpy_s(&clientInfo.clientInfo.data,
                                                   sizeof(clientInfo.clientInfo.data),
    ...
    Result MessageChannel<MsgTransport>::Send(ClientId dstClientId, Protocol protocol, MessageCode message, const ClientMetadata &metadata, uint32 payloadSizeInBytes, const void* pPayload)
    {
        ...
        if (payloadSizeInBytes <= kMaxPayloadSizeInBytes)
        {
            ...
            Platform::Memcpy_s(&messageBuffer.header.sequence,
                               sizeof(messageBuffer.header.sequence),
                               &metadata,
                               sizeof(Sequence));

            if ((pPayload != nullptr) & (payloadSizeInBytes != 0))
            {
                Platform::Memcpy_s(messageBuffer.payload,
                                   sizeof(messageBuffer.payload),
                                   pPayload,
                                   payloadSizeInBytes);
            }
