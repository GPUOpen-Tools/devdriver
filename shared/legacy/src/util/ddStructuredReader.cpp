...
#define ResetInternalErrorState() ResetInternalErrorStateImpl(DD_FILE, __LINE__, __FUNCTION__)
...
namespace DevDriver
{
    ...
    static mpack_node_t UnpackNode(const StructuredValue::OpaqueNode& opaque)
    {
        ...
        mpack_node_t node;
        Platform::Memcpy_s(&node, sizeof(node), &opaque, sizeof(node));
    ...
    static StructuredValue::OpaqueNode PackNode(mpack_node_t node)
    {
        ...
        Platform::Memcpy_s(reinterpret_cast<void*>(&opaque), sizeof(opaque), &node, sizeof(node));
