    ...
    const time_t* pFileCreateTime)
{
    ...
    struct tm rmtFileTime = {};

#ifdef _WIN32
    if (pFileCreateTime == nullptr)
    {
        time_t currTime = time(NULL);
        int filetimeResult = gmtime_s(&rmtFileTime, &currTime);
        DD_ASSERT(filetimeResult == 0);
        DD_UNUSED(filetimeResult);
    }
    else
    {
        int filetimeResult = gmtime_s(&rmtFileTime, pFileCreateTime);
        DD_ASSERT(filetimeResult == 0);
        DD_UNUSED(filetimeResult);
    }
#else
    ...
    {
        time_t currTime = time(NULL);
        struct tm* result = gmtime_r(&currTime, &rmtFileTime);
        DD_ASSERT(result != nullptr);
        DD_UNUSED(result);
    }
    else
    {
        struct tm* result = gmtime_r(pFileCreateTime, &rmtFileTime);
        DD_ASSERT(result != nullptr);
        DD_UNUSED(result);
    }
#endif
    ...
    fileHeader.second            = rmtFileTime.tm_sec;
    fileHeader.minute            = rmtFileTime.tm_min;
    fileHeader.hour              = rmtFileTime.tm_hour;
    fileHeader.dayInMonth        = rmtFileTime.tm_mday;
    fileHeader.month             = rmtFileTime.tm_mon;
    fileHeader.year              = rmtFileTime.tm_year;
    fileHeader.dayInWeek         = rmtFileTime.tm_wday;
    fileHeader.dayInYear         = rmtFileTime.tm_yday;
    fileHeader.isDaylightSavings = rmtFileTime.tm_isdst;
    ...
    uint64      snapshotTimestamp)
{
    ...
    const uint32 snapshotNameLength = static_cast<uint32>(Platform::Strlen_s(pSnapshotName, SIZE_MAX));
    ...
    size_t      dataSize)
{
    ...
    Platform::Memcpy_s(pDst, dataSize, pData, dataSize);
}
