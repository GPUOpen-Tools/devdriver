...
namespace DevDriver
{
    namespace Platform
    {
        ...
        {
            return __atomic_load_n(pVariable, __ATOMIC_SEQ_CST);
        }

        void AtomicSet(Atomic* pVariable, int32 num)
        {
            __atomic_store_n(pVariable, num, __ATOMIC_SEQ_CST);
        }
...
// function for non-Android platform.
        ...
        {
            DD_UNUSED(searchPaths);  // Linux: dlopen does not support equivalent search path restrictions.
        ...
        void GetProcessName(char* buffer, size_t bufferSize)
        {
            ...
            const char* pProcessName = (Platform::Strlen_s(GOverrideProcessName, kMaxStringLength) > 0) ?
                                       GOverrideProcessName : getprogname();
        ...
        void GetProcessName(char* buffer, size_t bufferSize)
        {
            ...
            // If the primary process-name source returns nullptr or an empty string, which can happen
            // when the process is running under Wine on Linux, try to read the process name from
            // /proc/self/exe instead.
            if ((pProcessName == nullptr) || (pProcessName[0] == '\0'))
            {
                char path[PATH_MAX] = {};
                ...
                if (len != -1)
                {
                    ...
                    pName = (pName != NULL) ? (pName + 1) : path;
                    Strncpy(programName, pName, sizeof(programName));

                    // If the executable name is "wine" or "wine64", it means we are running a Windows
                    // application through Wine on Linux. In this case, we can try to get the original
                    // Windows executable name from the cmdline.
                    if ((Strcmpi(pName, "wine") == 0) ||
                        (Strcmpi(pName, "wine64") == 0) ||
                        (Strcmpi(pName, "wine-preloader") == 0) ||
                        (Strcmpi(pName, "wine64-preloader") == 0))
                    {
                        ...
                        if (fp != NULL)
                        {
                            ...
                            // Append executable name to the program name, separated by a dash.
                            // For example, if the original program name is "wine" and the executable
                            // name is "mygame.exe", the final program name will be "wine-mygame.exe".
                            const char* pExeName = strrchr(exeName, '\\');
                            pExeName = (pExeName != NULL) ? (pExeName + 1) : exeName;
                            Strncat(programName, "-", sizeof(programName));
                            Strncat(programName, pExeName, sizeof(programName));

                            fclose(fp);
                        }
                    }

                    pProcessName = programName;
                }
            }
            ...
            Strncpy(buffer, pProcessName, bufferSize);
        }
#endif
        ...
        void Strncpy(char* pDst, const char* pSrc, size_t dstSize)
        {
            ...
            // Guard against dstSize == 0 to prevent underflow and out-of-bounds writes
            if (dstSize == 0)
            {
                return;
            }

            DD_WARN(Strlen_s(pSrc, SIZE_MAX) < dstSize);

// GCC's -Wstringop-truncation warns about intentional truncation.
// We explicitly null-terminate after strncpy, so this is safe.
...
#pragma GCC diagnostic pop
        ...
        void Strncat(char* pDst, const char* pSrc, size_t dstSize)
        {
            ...
            const size_t dstLength = Strlen_s(pDst, dstSize);
            strncat(pDst, pSrc, (dstSize - dstLength - 1));
        }

        void Memcpy_s(void* pDst, size_t dstSize, const void* pSrc, size_t srcSize)
        {
            // Match memcpy_s behavior: early return if srcSize is 0
            // This must come first - pSrc can be nullptr when srcSize is 0
            if (srcSize == 0)
            {
                return;
            }

            // Now assert on nullptr since we know srcSize > 0
            DD_ASSERT(pDst != nullptr);
            DD_ASSERT(pSrc != nullptr);
            DD_ASSERT(dstSize >= srcSize);

            // Match memcpy_s behavior: zero destination and return on error
            if ((pDst == nullptr) || (pSrc == nullptr) || (dstSize < srcSize))
            {
                if (pDst != nullptr)
                {
            ...
            std::copy(pSrcBytes, pSrcBytes + srcSize, pDstBytes);
        }
        ...
        void Memmove_s(void* pDst, size_t dstSize, const void* pSrc, size_t srcSize)
        {
            ...
            // Match memmove_s behavior: early return if srcSize is 0
            if (srcSize == 0)
            {
                return;
            }

            // Match memmove_s behavior: zero destination and return on error
            if ((pDst == nullptr) || (pSrc == nullptr) || (dstSize < srcSize))
            {
                if (pDst != nullptr)
                {
            ...
            // Check if regions overlap and destination is after source
            if ((pDstBytes > pSrcBytes) && (pDstBytes < (pSrcBytes + srcSize)))
            {
                // Use copy_backward for overlapping regions where dst > src
                std::copy_backward(pSrcBytes, pSrcBytes + srcSize, pDstBytes + srcSize);
            }
            else
            {
                std::copy(pSrcBytes, pSrcBytes + srcSize, pDstBytes);
        ...
        void Strerror_s(char* pBuf, size_t bufSize, int err)
        {
            ...
            // GNU variant returns char* which may point to a static string, not necessarily pBuf
            const char* pMsg = strerror_r(err, pBuf, bufSize);
            if (pMsg != pBuf)
            {
                strncpy(pBuf, pMsg, bufSize - 1);
                pBuf[bufSize - 1] = '\0';
            }
#else
            // POSIX variant always writes into pBuf
            strerror_r(err, pBuf, bufSize);
#endif
        }
        ...
        const char* SecureGetEnv(const char* name)
        {
            ...
            // On Linux (including Android), use secure_getenv if available (glibc >= 2.17)
            // This function returns nullptr if the process is running with elevated privileges
#if defined(__GLIBC__) && (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 17))
            return secure_getenv(name);
#else
            // Fallback for older glibc or Android: manually check for setuid/setgid
            // This mimics secure_getenv() behavior by rejecting environment variables
            // when running with elevated privileges (effective UID/GID differs from real UID/GID)
        ...
        Result QueryOsInfo(OsInfo* pInfo)
        {
            ...
            // Query OS name
            {
                ...
                if (pNameLine != nullptr)
                {
                    ...
                    char* pName = pNameLine + Strlen_s(OsNameLinePrefix, LineSize);
                    ...
                    if (nameLen == 0)
                    {
                        nameLen = Strlen_s(pName, LineSize);
                    }

                    size_t destNameSize = sizeof(pInfo->name) - 1;
                    size_t copySize = Platform::Min(destNameSize, nameLen);

                    // The string will be null-terminated in Strncpy
                    Strncpy(pInfo->name, pName, copySize + 1);
                }
            }
