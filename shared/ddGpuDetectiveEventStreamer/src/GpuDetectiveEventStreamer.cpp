    ...
    uint32_t        providerId)
{
    ...
    if (m_pStreamFile != nullptr)
    {
        ...
        m_pStreamFile   = nullptr;
        DevDriver::Platform::AtomicSet(&m_totalDataSize, 0);
    }
    ...
    uint32_t        providerId)
{
    ...
    if (result == DD_RESULT_SUCCESS)
    {
        m_pStreamFile = Platform::Tmpfile_s();
        if (m_pStreamFile == nullptr)
        {
...
DD_RESULT GPUDetectiveStreamer::EndStreaming(bool isClientAlive)
{
    ...
    {
        DevDriver::Platform::AtomicSet(&m_totalDataSize, static_cast<int64>(ftell(m_pStreamFile)));
    }
    else
    {
        DevDriver::Platform::AtomicSet(&m_totalDataSize, 0);
    ...
    bool                 useCompression)
{
    ...
    {
        const int64 totalDataSize = DevDriver::Platform::AtomicGet(&m_totalDataSize);
        if (totalDataSize > 0)
        {
            const uint32_t currPosition = ftell(m_pStreamFile);
            rewind(m_pStreamFile);
            size_t bytesRemaining       = (size_t)totalDataSize;

            rdfChunkCreateInfo chunkInfo = {};
            DevDriver::Platform::Memcpy_s(chunkInfo.identifier, RDF_IDENTIFIER_SIZE, kDevDriverEventChunkId, RDF_IDENTIFIER_SIZE);
